namespace Game2DWaterKit
{
    ...
    public abstract class Game2DWaterKitShaderGUI : ShaderGUI
    {
        ...
        private static SerializedObject _projectGrahicsSettings;
        private static Texture2D[] _noiseTexturePreviews = new Texture2D[4];
        private static Dictionary<string, AnimBool> _foldoutAnimBools = new Dictionary<string, AnimBool>();
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            DrawShaderMissingInProjectAlwaysIncludedShadersWarning();
        ...
        protected void DrawPropertiesGroup(string groupName, System.Action propertiesLayoutProc, MaterialProperty groupKeywordState = null)
        {
            ...
            else
            {
                ...
                rect.xMax = xmax;
                EditorGUI.BeginChangeCheck();
                var foldout = EditorGUI.Foldout(rect, fadeGroupAnimBool.value, "    " + groupName, true);
                if (EditorGUI.EndChangeCheck())
                    fadeGroupAnimBool.target = foldout;
            }
        ...
        private void DrawShaderMissingInProjectAlwaysIncludedShadersWarning()
        {
            if (!AssetDatabase.Contains(_material))
                return;
            
            if (_projectGrahicsSettings == null)
                _projectGrahicsSettings = new SerializedObject(AssetDatabase.LoadAssetAtPath<GraphicsSettings>("ProjectSettings/GraphicsSettings.asset"));
            
            _projectGrahicsSettings.UpdateIfRequiredOrScript();

            var alwaysIncludedShadersProperty = _projectGrahicsSettings.FindProperty("m_AlwaysIncludedShaders");
            bool isShaderIncluded = false;
            for (int i = 0, imax = alwaysIncludedShadersProperty.arraySize; i < imax; i++)
            {
                if (_shader == alwaysIncludedShadersProperty.GetArrayElementAtIndex(i).objectReferenceValue)
                {
                    isShaderIncluded = true;
                    break;
                }
            }
            ...
            {
                BeginBoxGroup();
                string missingShaderMsg = string.Format("The shader \"{0}\" might be not included into the player build if nothing in your scenes references it!\nIn case you experience the pink \"missing material\" in the player build, please add this shader to your Project \"Always Included Shaders\".", _shader.name);
                EditorGUILayout.HelpBox(missingShaderMsg, MessageType.Info);
                
                if (GUILayout.Button("Add Shader to Project \"Always Included Shaders\""))
                {
                    int index = alwaysIncludedShadersProperty.arraySize;
                    alwaysIncludedShadersProperty.InsertArrayElementAtIndex(index);
                    alwaysIncludedShadersProperty.GetArrayElementAtIndex(index).objectReferenceValue = _shader;

                    _projectGrahicsSettings.ApplyModifiedPropertiesWithoutUndo();
        ...
        private static AnimBool GetFoldoutAnimBool(string name)
        {
            ...
            if (!_foldoutAnimBools.TryGetValue(name, out foldoutAnimBool))
            {
                foldoutAnimBool = new AnimBool(() => _foldoutAnimBoolsAction());
                _foldoutAnimBools.Add(name, foldoutAnimBool);
            }
